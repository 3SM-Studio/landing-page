...
export function WorkCard({ locale, item, viewProjectLabel }: WorkCardProps) {
  return (
    <Link
      href={{
        pathname: '/work/[slug]',
    ...
    >
      <WorkMeta locale={locale} category={item.category} year={item.year} size="card" />
      ...
      <div className="flex items-center justify-between border-t border-white/10 pt-5">
        <span className="text-sm text-slate-500">
          {item.category ? getWorkCategoryLabel(item.category, locale) : ''}
        </span>

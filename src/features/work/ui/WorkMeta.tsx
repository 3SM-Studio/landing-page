...
export function WorkMeta({ locale, category, year, size = 'card' }: WorkMetaProps) {
  const className =
    size === 'card'
  ...
  return (
    <div className={className}>
      {category ? <span>{getWorkCategoryLabel(category, locale)}</span> : null}

      {category && year ? <span className="h-1 w-1 rounded-full bg-slate-500" /> : null}

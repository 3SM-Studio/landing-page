...
import { CASE_STUDIES_REVALIDATE_SECONDS } from '../model/case-studies.constants';
...
const caseStudiesFetchOptions = {
  revalidate: CASE_STUDIES_REVALIDATE_SECONDS,
  tags: [sanityTags.caseStudies],
} as const;

export async function getCaseStudies(locale: Locale) {
  const items = await sanityFetch<RawCaseStudy[]>(
    CASE_STUDIES_QUERY,
    { locale },
    caseStudiesFetchOptions,
  );
...
export async function getCaseStudyBySlug(locale: Locale, slug: string) {
  const item = await sanityFetch<RawCaseStudy | null>(
    CASE_STUDY_BY_SLUG_QUERY,
    { locale, slug },
    caseStudiesFetchOptions,
  );
...
export async function getCaseStudySlugs(locale: Locale) {
  const items = await sanityFetch<RawCaseStudySlug[]>(
    CASE_STUDY_SLUGS_QUERY,
    { locale },
    caseStudiesFetchOptions,
  );
